...
### 单机模式
- 8 次猜测机会，猜出隐藏的 F1 车手
- 经典版 109 位车手 / 简单版 54 位（2022 年后出场 + 世界冠军传奇）
- 2026 赛季数据，战绩按模式分开统计（localStorage）
- 颜色反馈（绿/黄/灰）+ 数值方向箭头，搜索支持音调归一化（é→e）
- 手机端卡片布局，桌面端表格布局
...
- 防作弊（游戏结束前不发送目标车手 ID，服务端校验猜测在池内）
...
- 闲置房间自动销毁（DO alarm），降低 Durable Objects 时长消耗
- 搜索音调归一化（Sébastien 可用 sebastien 搜到）
- 公告最多展示最近两条；弹窗去掉 backdrop-blur 修复低端设备掉帧
...
- 闲置销毁：等待 5 分钟无人加入 / 结算后闲置 5 分钟，自动关闭（DO alarm）
- 断线重连：会话存 sessionStorage，刷新/断网自动以同一 playerId 恢复；30 秒未重连判负
